...
dataset = pd.read_csv("./data/twitter.csv") # 훈련용 말뭉치 생성
...
embedding_weights = torch.Tensor(word_vectors.vectors) # LSTM 입력에 의미값(semantic value)를 부여하기 위한 임베딩
...
batch_size = 2 # 컴퓨터가 처리하기 쉽도록 2의 거듭제곱으로 설정
...
for model in [rnn_model, lstm_model]:
	...
	if "RNN" in model.__class__.__name__:
		for epoch in range(N_EPOCHS):
			train_loss, train_acc = train(rnn_model, train_iterator, rnn_optimizer, rnn_criterion)
			print(
				f"| Epoch: {epoch + 1:02} | Train Loss: {train_loss: .3f} | Train Acc: {train_acc * 100: .2f}% |"
			)
	else:
		for epoch in range(N_EPOCHS):
			train_loss, train_acc = train(lstm_model, train_iterator, lstm_optimizer, lstm_criterion)
			valid_loss, valid_acc = evaluate(lstm_model, validate_iterator, lstm_criterion)
			print(
